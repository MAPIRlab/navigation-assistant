...
Action string_to_enum(std::string action_str)
{
    ...
    {
        RCLCPP_ERROR(rclcpp::get_logger("topology_graph"), "Requested action not implemented.");
...
CGraphWrapper::CGraphWrapper() : Node("Topology_graph")
{
    ...
    makePlanNode = std::make_shared<rclcpp::Node>("makePlanNode");
    ...
    getPlanClient = rclcpp_action::create_client<GetPlan>(makePlanNode, get_plan_server);

    using namespace std::chrono_literals;
    while (rclcpp::ok() && !getPlanClient->wait_for_action_server(5.0s))
        RCLCPP_WARN(get_logger(), "[topology_graph] Waiting for move_base MAKE_PLAN srv to come online.");
    ...
    free_paths.clear();
    num_path = 0;
    RCLCPP_INFO(get_logger(), "[topology_graph] Ready for Operation");
}
...
bool CGraphWrapper::srvCB(topology_graph::srv::Graph::Request::SharedPtr req, topology_graph::srv::Graph::Response::SharedPtr res)
{
    ...
    case AddNode:
        ...
        if (req->params.size() == 4 || req->params.size() == 5)
        {
            ...
            if (my_graph.AddNode(req->params[0], req->params[1], pos_x, pos_y, pos_yaw, node_id))
            {
                ...
                if (verbose)
                    RCLCPP_INFO(get_logger(), "[topology_graph-AddNode] New node added with id=%lu", node_id);
            }
            else
            {
                res->success = false;
                if (verbose)
                    RCLCPP_WARN(get_logger(), "[topology_graph-AddNode] Error adding new node.");
            }
        }
        else if (verbose)
            RCLCPP_WARN(get_logger(), "[topology_graph-AddNode] Incorrect Number of parameters (4 required)");
    ...
    case AddArc:
        ...
        if (req->params.size() == 5)
        {
            ...
            bool bidi =
                (req->params[4] == "yes" || req->params[4] == "Yes" || req->params[4] == "true" || req->params[4] == "True" || req->params[4] == "1");
            ...
            if (my_graph.AddArc(id_from, id_to, req->params[2], req->params[3], bidi, arc_id))
            {
                ...
                if (verbose)
                    RCLCPP_INFO(get_logger(), "[topology_graph-AddArc] New Arc added with id=%lu", arc_id);
            }
            else
            {
                res->success = false;
                if (verbose)
                    RCLCPP_WARN(get_logger(), "[topology_graph-AddArc] Error adding new arc.");
            }
        }
        else if (verbose)
            RCLCPP_WARN(get_logger(), "[topology_graph-AddArc] Error: Incorrect Number of parameters (5 required)");
    ...
    case AddArcbyLabel:
        ...
        if (req->params.size() == 5)
        {
            ...
            if (my_graph.AddArcbyLabel(req->params[0], req->params[1], req->params[2], req->params[3], bidi, arc_id))
            {
                ...
                res->result.push_back(std::to_string(arc_id));
                if (verbose)
                    RCLCPP_INFO(get_logger(), "[topology_graph-AddArcbyLabel] New Arc added with id=%lu", arc_id);
            }
            else
            {
                res->success = false;
                if (verbose)
                    RCLCPP_WARN(get_logger(), "[topology_graph-AddArcbyLabel] Error adding new arc.");
            }
        }
        else
        {
            if (verbose)
                RCLCPP_WARN(get_logger(), "[topology_graph-AddArcbyLabel] Error: Incorrect Number of parameters (5 required)");
        }
    ...
    case FindPath:
        ...
        try
        {
            if (req->params.size() == 2)
            {
                ...
                if (my_graph.FindPath(id_start, id_end, path))
                {
                    ...
                    res->result = path;
                    if (verbose)
                        RCLCPP_INFO(get_logger(), "[topology_graph-FindPathInfo] Find path [%s]-->[%s] done. ", req->params[0].c_str(),
                                    req->params[1].c_str());
                }
                else
                {
                    res->success = false;
                    if (verbose)
                        RCLCPP_WARN(get_logger(), "[topology_graph-FindPathInfo] Error Finding path [%s]-->[%s] ", req->params[0].c_str(),
                                    req->params[1].c_str());
                }
            }
            else
                RCLCPP_WARN(get_logger(), "[topology_graph-FindPathInfo]Error: Incorrect Number of parameters (2 required)");
        }
        catch (exception e)
        {
            RCLCPP_ERROR(get_logger(), "[topology_graph-FindPathInfo] Exception: %s\n", e.what());
            res->result.clear();
            res->success = false;
            return true;
        }
        ...
        {
            RCLCPP_ERROR(get_logger(), "[topology_graph-FindPathInfo] Unknown Exception");
            res->result.clear();
            res->success = false;
            return true;
        }
    ...
    case GetNavDistTwoPoses:
        ...
        try
        {
            if (req->params.size() >= 6)
            {
                ...
                RCLCPP_WARN(get_logger(), "[topology_graph-GetNavDistTwoPoses]Error: Incorrect Number of parameters (+6 required)");
        }
        catch (exception e)
        {
            RCLCPP_ERROR(get_logger(), "[topology_graph-GetNavDistTwoPoses] Exception: %s\n", e.what());
            res->result.clear();
            res->success = false;
            return true;
        }
        ...
        {
            RCLCPP_ERROR(get_logger(), "[topology_graph-GetNavDistTwoPoses] Unknown Exception");
            res->result.clear();
            res->success = false;
            return true;
        }
    ...
    case LoadGraph:
        ...
        {
            if (verbose)
                RCLCPP_INFO(get_logger(), "[topology_graph-LoadGraph] Error loading graph from file.");
            res->success = false;
        }
        break;
    ...
    case SaveGraph:
        ...
        if (req->params.size() == 1)
        {
            ...
            if (verbose)
                RCLCPP_INFO(get_logger(), "[topology_graph-SaveGraph] Saving graph to file done");
            res->success = true;
        }
        else
        {
            if (verbose)
                RCLCPP_INFO(get_logger(), "[topology_graph-SAveGraph] Error saving graph to file. Incorrect Number of parameters (1 required)");
            res->success = false;
        }
        break;
    ...
    case GetNodesbyLabel:
        ...
        if (req->params.size() == 1)
        {
            ...
            res->result = nodeList;
            if (verbose)
                RCLCPP_INFO(get_logger(), "[topology_graph-GetNodesbyLabel] Done");
        }
        else if (verbose)
            RCLCPP_INFO(get_logger(), "[topology_graph-GetNodesbyLabel] Error: Incorrect Number of parameters (1 required)");
    ...
    case GetNodesbyType:
        ...
        if (req->params.size() == 1)
        {
            ...
            res->result = nodeList;
            if (verbose)
                RCLCPP_INFO(get_logger(), "[topology_graph-GetNodesbyType] Done");
        }
        else if (verbose)
            RCLCPP_INFO(get_logger(), "[topology_graph-GetNodesbyType] Error: Incorrect Number of parameters (1 required)");
    ...
    case GetNodesbyLabelandType:
        ...
        if (req->params.size() == 2)
        {
            ...
            res->result = nodeList;
            if (verbose)
                RCLCPP_INFO(get_logger(), "[topology_graph-GetNodesbyLabelandType] Done");
        }
        else if (verbose)
            RCLCPP_INFO(get_logger(), "[topology_graph-GetNodesbyLabelandType] Error: Incorrect Number of parameters (2 required)");
    ...
    case GetNodebyId:
        ...
        if (req->params.size() == 1)
        {
            ...
            if (node_data != "")
            {
                ...
                if (verbose)
                    RCLCPP_INFO(get_logger(), "[topology_graph-GetNodeLabel] Done");
            }
            else
            {
                res->success = false;
                if (verbose)
                    RCLCPP_INFO(get_logger(), "[topology_graph-GetNodeLabel] Error: Node ID not found");
            }
        }
        else
            RCLCPP_INFO(get_logger(), "[topology_graph-GetNodeLabel] Error: Incorrect Number of parameters (1 required)");
    ...
    case GetAllNodes: {
        ...
        res->result = nodeList;
        if (verbose)
            RCLCPP_INFO(get_logger(), "[topology_graph-GetAllNodes] Done");
    }
    break;
    ...
    case GetAllArcs: {
        ...
        res->success = true;
        res->result = arcList;
        RCLCPP_INFO(get_logger(), "[topology_graph-GetAllArcs] Done");
    }
    break;
    ...
    case GetClosestNode:
        ...
        try
        {
            ...
            {
                if (verbose)
                    RCLCPP_WARN(get_logger(), "[topology_graph-GetClosestNode] Error: Incorrect Number of parameters (3 required - 5 alowed)");
                // Srv response
                res->success = false;
                res->result.clear();
                return true;
            ...
            else
            {
                ...
                for (auto n : node_list)
                {
                    ...
                    if (node_data.size() != 6)
                    {
                        RCLCPP_WARN(get_logger(), "[topology_graph-GetClosestNode] Error incorrect node data, skipping it.");
                    ...
                    if (verbose)
                        RCLCPP_INFO(get_logger(),
                                    "[topology_graph-GetClosestNode] Nav Distance between pose [%.2f, %.2f] and Node(%s)=[%.2f, %.2f] is %.3f[m]",
                ...
                if (first_candidate)
                {
                    ...
                    if (verbose)
                        RCLCPP_INFO(get_logger(), "[topology_graph-GetClosestNode] No closet node found. Error!\n\n");
                    res->success = false;
                    res->result.clear();
                }
                else
                {
                    if (verbose)
                        RCLCPP_INFO(get_logger(), "[topology_graph-GetClosestNode] Closet node is [%s]=%s.\n\n", closest_node[0].c_str(),
                                    closest_node[1].c_str());
                    res->success = true;
        ...
        {
            RCLCPP_ERROR(get_logger(), "[topology_graph-GetClosestNode] Exception: %s\n", e.what());
            res->result.clear();
            res->success = false;
            return true;
        }
        ...
        {
            RCLCPP_ERROR(get_logger(), "[topology_graph-GetClosestNode] Unknown Exception");
            res->result.clear();
            res->success = false;
            return true;
        }
    ...
    case DeleteNodeByLabel:
        ...
        if (req->params.size() == 1)
        {
            ...
            {
                res->success = true;
                RCLCPP_INFO(get_logger(), "[topology_graph-DeleteNode] Node has been deleted.");
            }
            else
            {
                res->success = false;
                RCLCPP_INFO(get_logger(), "[topology_graph-DeleteNode] Node not found. Unable to delete node.");
            }
        }
        else
        {
            res->success = false;
            RCLCPP_INFO(get_logger(), "[topology_graph-DeleteNode] Error: Incorrect Number of parameters (1 required)");
        }
        break;
    ...
    case DeleteAllArcs:
        ...
        if (req->params.size() == 1)
        {
            ...
            res->success = true;
            RCLCPP_INFO(get_logger(), "[topology_graph-DeleteAllArcs] Arcs have been deleted.");
        }
        else
        {
            res->success = false;
            RCLCPP_INFO(get_logger(), "[topology_graph-DeleteAllArcs] Error: Incorrect Number of parameters (1 required)");
        }
        break;

    default: {
        RCLCPP_ERROR(get_logger(), "[topology_graph] cmd not implemented!");
        return false;
    }
    } // end swith
...
/*
        ...
        case SetNodeLabel: // params: [id,label]
...

            {
                ...
                RCLCPP_INFO(get_logger(), "[topology_graph - SetNodeLabel]La accion SetNodeLabel se ha realizado con exito");
...

            {
                ...
                RCLCPP_INFO(get_logger(), "[topology_graph - SetNodeLabel]Error: Incorrect Number of parameters (2 required)");
        ...
        case GetNodeNeighbors: // params: [idnode,arc_type]
...

            {
                ...
                RCLCPP_INFO(get_logger(), "[topology_graph - GetNodeNeighbors]La accion GetNodeNeighbors se ha realizado con exito");
            }
            else

            {

                RCLCPP_INFO(get_logger(), "[topology_graph - GetNodeNeighbors]Error: Incorrect Number of parameters (2 required)");
        ...
        case GetNodeByLocation: // params: [x,y,tolerance]
...

            {
                ...
                RCLCPP_INFO(get_logger(), "[topology_graph - GetNodeByLocation]La accion GetNodeByLocation se ha realizado con exito");
...

            {
                ...
                RCLCPP_INFO(get_logger(), "[topology_graph - GetNodeByLocation]Error: Incorrect Number of parameters (3 required)");
        ...
        case DeleteOutcommingArcs: // params: [idnode,arc_type]
...

            {
                ...
                RCLCPP_INFO(get_logger(), "[topology_graph - DeleteOutcommingArcs]La accion DeleteOutcommingArcs se ha realizado con exito");
...

            {
                ...
                RCLCPP_INFO(get_logger(), "[topology_graph - DeleteOutcommingArcs]Error: Incorrect Number of parameters (2 required)");
        ...
        case DeleteIncommingArcs: // params: [idnode,arc_type]
...

            {
                ...
                RCLCPP_INFO(get_logger(), "[topology_graph - DeleteIncommingArcs]La accion DeleteIncommingArcs se ha realizado con exito");
...

            {
                ...
                RCLCPP_INFO(get_logger(), "[topology_graph - DeleteIncommingArcs]Error: Incorrect Number of parameters (2 required)");
        ...
        case DeleteAllArcs: // params: [idnode,arc_type /arc_type]
...

            {
                ...
                RCLCPP_INFO(get_logger(), "[topology_graph - DeleteAllArcs]La accion DeleteAllArcs se ha realizado con exito");
...

            {
                ...
                RCLCPP_INFO(get_logger(), "[topology_graph - DeleteAllArcs]Error: Incorrect Number of parameters (1 or 2 required)");
        ...
        case DeleteArcsBetweenNodes: // params: [idfrom,idto,type]
...

            {
                ...
                RCLCPP_INFO(get_logger(), "[topology_graph - DeleteArcsBetweenNodes]La accion DeleteArcsBetweenNodes se ha realizado con exito");
...

            {
                ...
                RCLCPP_INFO(get_logger(), "[topology_graph - DeleteArcsBetweenNodes]Error: Incorrect Number of parameters (3 required)");
        ...
        case SetNodeLocation: // params: [node_label,x,y]
...

            {
                ...
                RCLCPP_INFO(get_logger(), "[topology_graph - SetNodeLocation]La accion SetNodeLocation se ha realizado con exito");
...

            {
                ...
                RCLCPP_INFO(get_logger(), "[topology_graph - SetNodeLocation]Error: Incorrect Number of parameters (3 required)");
        ...
        case GetNodeLocation: // params: [node_label]
...

            {
                ...
                RCLCPP_INFO(get_logger(), "[topology_graph - GetNodeLocation]La accion GetNodeLocation se ha realizado con exito");
...

            {
                ...
                RCLCPP_INFO(get_logger(), "[topology_graph - GetNodeLocation]Error: Incorrect Number of parameters (1 required)");
        ...
        case ExistsNodeLabel: // params: [label]
...

            {
                ...
                RCLCPP_INFO(get_logger(), "[topology_graph - ExistsNodeLabel]La accion ExistsNodeLabel se ha realizado con exito");
...

            {
                ...
                RCLCPP_INFO(get_logger(), "[topology_graph - ExistsNodeLabel]Error: Incorrect Number of parameters (1 required)");
        ...
        case AddArcRvizTool:
            ...
            if(req->params.size()==4)
            {
               ...
               my_graph.GetNodeLocation(label_init,x_node_init,y_node_init); // En este punto,ya se han obtenido todos los datos del nodo origen
   necesarios.
               ...
               my_graph.GetNodeLocation(label_fin,x_node_fin,y_node_fin); // En este punto,ya se han obtenido todos los datos del nodo destino
   necesarios.
               ...
               RCLCPP_INFO(get_logger(), "[topology_graph - AddArcRvizTool]La accion AddArcRvizTool se ha realizado con exito");
        ...
        case DeleteNodeRvizTool:
            ...
            if (req->params.size()==2)
            {
                ...
                RCLCPP_INFO(get_logger(), "[topology_graph - DeleteNodeRvizTool]La accion DeleteNodeRvizTool se ha realizado con exito");
        ...
        case PrintGraph:
           ...
           if(req->params.size()==0)
           {
              ...
              RCLCPP_INFO(get_logger(), "[topology_graph - PrintGraph]La accion PrintGraph se ha realizado con exito");
...
bool CGraphWrapper::get_line_intersection(float p0_x, float p0_y, float p1_x, float p1_y, float p2_x, float p2_y, float p3_x, float p3_y, float* i_x,
                                          float* i_y)
{
    ...
    try
    {
        ...
        if (s >= 0 && s <= 1 && t >= 0 && t <= 1)
        {
            ...
            // if (verbose) RCLCPP_INFO(get_logger(), "[topology_graph] get_line_intersection = TRUE");
            return true;
        }

        // if (verbose) RCLCPP_INFO(get_logger(), "[topology_graph] get_line_intersection = FALSE");
        return false; // No collision
    }
    catch (exception e)
    {
        RCLCPP_ERROR(get_logger(), "[topology_graph-get_line_intersection] Exception: %s\n", e.what());
        return false;
    }
    catch (...)
    {
        RCLCPP_ERROR(get_logger(), "[topology_graph-get_line_intersection] Unknown Exception");
        return false;
    }
}

bool CGraphWrapper::segment_intersect_node(geometry_msgs::msg::Pose segment_ini, geometry_msgs::msg::Pose segment_end,
                                           std::vector<std::string> sp_list)
{
    try
    {
        ...
        for (size_t idx = 0; idx < sp_list.size(); idx += 2)
        {
            ...
            if (sp1[1] != sp2[1])
            {
                RCLCPP_ERROR(get_logger(), "[topology_graph-segment_intersect_node] Two SP have different labels. This should NOT happen.");
                return false;
            }
            ...
            bool doIntersect = get_line_intersection(seg_ini.x, seg_ini.y, seg_end.x, seg_end.y, segment_ini.position.x, segment_ini.position.y,
                                                     segment_end.position.x, segment_end.position.y, &intersec_x, &intersec_y);
    ...
    {
        RCLCPP_ERROR(get_logger(), "[topology_graph-segment_intersect_node] Exception: %s\n", e.what());
        return false;
    }
}
...
// -----------------------------------------------------------------------------
double CGraphWrapper::get_nav_distance_two_poses(geometry_msgs::msg::Pose pose_origin, geometry_msgs::msg::Pose pose_goal,
                                                 std::vector<std::string> avoiding_node_types)
{
    try
    {
        if (verbose)
            RCLCPP_INFO(get_logger(), "Trying to find distance between (%.2f, %.2f) and (%.2f, %.2f)", pose_origin.position.x, pose_origin.position.y,
        ...
        std::optional<nav_msgs::msg::Path> actionReponse = std::nullopt;
        static auto result_cb = [&](const rclcpp_action::ClientGoalHandle<GetPlan>::WrappedResult w_result) {
            actionReponse = w_result.result->path;
        };
        ...
        auto result = rclcpp::spin_until_future_complete(makePlanNode, future, std::chrono::seconds(1));
        // Check if three is a valid path between both poses
        if (result != rclcpp::FutureReturnCode::SUCCESS)
        {
            // SRV is not available!! Report Error
            RCLCPP_ERROR(get_logger(), "[topology_graph] Unable to call MAKE_PLAN service from MoveBase");
            return -1.0;
        }
        ...
        while (!actionReponse.has_value())
        {
            wait_rate.sleep();
            rclcpp::spin_some(makePlanNode);
        }

        nav_msgs::msg::Path plan = actionReponse.value();
        ...
        if (plan.poses.empty())
        {
            ...
            if (verbose)
                RCLCPP_WARN(get_logger(),
                            "[topology_graph] MAKE_PLAN returned empty. Unable to find a path between poses [%.2f, %.2f] and [%.2f, %.2f]",
        ...
        float distanceToStart =
            sqrt(pow(pose_origin.position.x - plan.poses[0].pose.position.x, 2) + pow(pose_origin.position.y - plan.poses[0].pose.position.y, 2));
        ...
        double Ax = 0.0, Ay = 0.0, d = 0.0, total_nav_dist = 0.0;
        geometry_msgs::msg::Pose p_ini = request.start.pose;
        geometry_msgs::msg::Pose p_end;
        for (; planPoseIndex < plan.poses.size(); planPoseIndex++)
        {
            ...
            if (d >= dist_step)
            {
                p_end = plan.poses[planPoseIndex].pose;
            ...
            p_ini = plan.poses[planPoseIndex].pose;
        }
    ...
    {
        RCLCPP_ERROR(get_logger(), "[topology_graph-%s] Exception when updating TOPOLOGICAL POSE: [%s]", __FUNCTION__, ex.what());
        return -1;
    }
}
...
void CGraphWrapper::DrawSegment_intersection(geometry_msgs::msg::Point p1, geometry_msgs::msg::Point q1, geometry_msgs::msg::Point p2,
...
void CGraphWrapper::publishGraphAsMarkers()
{
    try
    {
        ...
        for (auto n : node_list)
        {
            ...
            {
                if (verbose)
                    RCLCPP_WARN(get_logger(), "[topology_graph-publishGraphAsMarkers] Error incorrect node data, skipping it.");
                for (auto i : node_data)
                    std::cout << i << " ";
                continue;
            }
            ...
            // draw node
            // if (verbose) RCLCPP_INFO(get_logger(), "[topology_graph] Drawing new node");
            DrawNode(atoi(node_data[0].c_str()), node_data[1], node_data[2], atof(node_data[3].c_str()), atof(node_data[4].c_str()),
                     atof(node_data[5].c_str()));
        }
        ...
        for (auto n : arc_list)
        {
            ...
            {
                if (verbose)
                    RCLCPP_WARN(get_logger(), "[topology_graph-publishGraphAsMarkers] Error incorrect ARC data, skipping it.");
                continue;
            }
            ...
            // draw arc
            // if (verbose) RCLCPP_INFO(get_logger(), "[topology_graph] Drawing new arc");
        ...
        if (num_path < free_paths.size())
        {
            ...
            RCLCPP_INFO(get_logger(), "[Graph] publishing path %i", num_path);
            num_path ++;
        }
        else
        {
    ...
    {
        RCLCPP_ERROR(get_logger(), "[topology_graph-%s] Exception: [%s]", __FUNCTION__, ex.what());
        return;
    }
    catch (exception& ex)
    {
        RCLCPP_ERROR(get_logger(), "[topology_graph-%s] - Error: %s", __FUNCTION__, ex.what());
        return;
    }
    catch (...)
    {
        RCLCPP_ERROR(get_logger(), "[topology_graph-%s] Unknown Exception", __FUNCTION__);
        return;
    }
}
...
{
    RCLCPP_INFO(rclcpp::get_logger("topology_graph"), "Initializing node... please wait!");
