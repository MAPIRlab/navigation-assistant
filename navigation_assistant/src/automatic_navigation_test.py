...
class NavTest(object):
    ...
    def __init__(self):
        ...
        rospy.Subscriber("/stage_stall", Int8, self.stage_stall_cb)
        ...
        rospy.wait_for_service('/navigation_assistant/add_point_of_interest')
        ...
        # START TALK
        if self.verbose: rospy.loginfo( "[Navigation_Test] Ready for Action." )
        if self.talk: self.say_voice("Hello. I am the Navigation Assistant. I am getting ready to perform a Navigation test ", 5.0)
        if self.talk: self.say_voice("Place on the map all the navigation goals that you want me to reach, and press the Green button to start the test. ", 5.0)
        ...
        #------------------------------
        if self.verbose: rospy.loginfo( "[Navigation_Test] Waiting GREEN button press." )
        ...
        # Talk
        if self.talk: self.say_voice("I have detected a total of " + str(len(self.nodes)) + "spaces or rooms in this environment.", 4.0)
        if self.talk: self.say_voice("I will now proceed to test navigation to ensure that I can reach every one of them.", 2.0)
        n = self.nodes[0].split()  # n = [id, label, type, x, y, yaw]
        if self.talk: self.say_voice("When you are ready, please, take me to " + n[1] + " and press the Green Button. At any moment you can add a Critial Navigation Point by pressing the Red button.", 1.0)

        # 2. Wait for User Confirmation
        #------------------------------
        if self.verbose: rospy.loginfo( "[Navigation_Test] Waiting GREEN button press." )
        if self.talk: self.wait_for_green_button()
        ...
        while n1 < len(self.nodes)-1:
            ...
            while n2 < len(self.nodes):
                ...
                while (not nav_completed) and (num_attemp <= max_attemps):
                    if self.talk: self.say_voice("Navigating to " + n2_data[1] + " .Attemp " + str(num_attemp), 2.0)
                    if self.verbose: rospy.loginfo( "[Navigation_Test] Navigating to " + n2_data[1] + " . Attemp " + str(num_attemp) )
                    ...
                    if nav_completed:
                        ...
                        if self.verbose: rospy.loginfo( "[Navigation_Test] Navigation comleted with success." )
                        self.under_collision = False    # ensure collision is off
                    else:
                        # Attemp to solve the issue
                        if self.propose_CNP():
                            self.say_voice("I am unable to reach my goal for some reason. I will try to solve this problem automatically.", 1.0)
                            if self.verbose: rospy.loginfo( "[Navigation_Test] Navigation failed. Trying Automatic approach." )

                            # Move the robot back (a little)
                            self.move_back()
                            rospy.sleep(2)

                        # Request User help
                        elif num_attemp < max_attemps:
                            self.say_voice("I am unable to reach my goal for some reason. Please revise the Critical Navigation Points and press the green button to try again.", 3.0)
                            if self.verbose: rospy.loginfo( "[Navigation_Test] User help requested. Press the green button." )
                            self.wait_for_green_button()
                            num_attemp += 1
                            self.under_collision = False    # ensure collision is off
                        else:
                            ...
                            if self.verbose: rospy.loginfo( "[Navigation_Test] Please take me to " + n2_data[1] + " and press the green button." )
                            self.wait_for_green_button()
                            num_attemp += 1
                            self.under_collision = False    # ensure collision is off
                ...
                self.robot_pose_history = []
                while (not nav_completed) and (num_attemp <= max_attemps):
                    if self.talk: self.say_voice("Navigating back to " + n1_data[1] + " . Attemp " + str(num_attemp), 2.0)
                    if self.verbose: rospy.loginfo( "[Navigation_Test] Navigating to " + n1_data[1] + " . Attemp " + str(num_attemp) )
                    ...
                    if nav_completed:
                        ...
                        # Attemp to solve the issue
                        if self.propose_CNP():
                            if self.talk: self.say_voice("I am unable to reach my goal for some reason. I will try to solve this problem automatically.", 1.0)
                            if self.verbose: rospy.loginfo( "[Navigation_Test] Navigation failed. Trying Automatic approach." )
                        ...
                        if num_attemp < max_attemps:
                            self.say_voice("I am unable to reach my goal for some reason. Please revise the Critical Navigation Point and press the green button to try again.", 3.0)
                            if self.verbose: rospy.loginfo( "[Navigation_Test] User help requested. Press the green button." )
                            self.wait_for_green_button()
                            num_attemp += 1
                            self.under_collision = False    # ensure collision is off
                            ...
                            if self.verbose: rospy.loginfo( "[Navigation_Test] Please take me to " + n1_data[1] + " and press the green button." )
                            self.wait_for_green_button()
                            num_attemp += 1
                            self.under_collision = False    # ensure collision is off
            ...
            if n1 < len(self.nodes)-1:
                ...
                self.robot_pose_history = []
                while (not nav_completed) and (num_attemp <= max_attemps):
                    if self.talk: self.say_voice("To continue the Navigation Test I need to go to " + n1_data[1] + " . Attemp " + str(num_attemp), 2.0)
                    if self.verbose: rospy.loginfo( "[Navigation_Test] To continue the Navigation Test I need to go to " + n1_data[1] + " . Attemp " + str(num_attemp) )
                    ...
                    if nav_completed:
                        ...
                        if num_attemp < max_attemps:
                            ...
                            if self.verbose: rospy.loginfo( "[Navigation_Test] Please help me a little bit and press the green button." )
                            self.wait_for_green_button()
                            num_attemp += 1
                            self.under_collision = False    # ensure collision is off
                        else:
    ...
    def propose_CNP(self):
        ...
        # Set seed at current robot location: geometry_msgs/PoseWithCovarianceStamped
        rospy.loginfo("New Seed to CNP at pose (x=%.3f, y=%.3f)[m]", self.robot_pose.pose.pose.position.x, self.robot_pose.pose.pose.position.y )

        # Get global costmap as img arround robot location
        img, top_left_idx = self.crop_costmap_as_img( self.pose_to_cell(self.robot_pose) )
        ...
        # CNP already exist or unable to set it
        return False
    ...
    def get_CNP_from_img(self, img):
        ...
        #-------------------------------
        rospy.loginfo("Selecting best contours")
        selected_contours = []
        min_d = 0.0
        Pcenter = []
        for c in filtered_contours:
        ...
        for c in contours:
            if len(selected_contours) == 0:
                ...
                if len(c) > len(selected_contours[0]):
                    selected_contours.append(c)
                else:
                    selected_contours.insert(0,c)
            else:
    ...
    #                    STAGE COLLISION                     #
    # ====================================================== #
    def stage_stall_cb(self, msg):
        if msg.data == 1 and self.under_collision==False:
            self.under_collision = True
            rospy.loginfo("[Navigation Test] Collision Detected!.")
    ...
    def close_and_return(self):
        ...
        # STOP THE ROBOT
        self.nav_client.cancel_all_goals()
        self.say_voice("Canceling navigation by request.", 2.0)
    ...
    # ====================================================== #
    def move_back(self):
        # Set cmd_vel command directly to move backwards (dangerous)
        twist_msg = Twist()
        twist_msg.linear.y = twist_msg.linear.z = 0

        # Stop the robot
        start = rospy.Time.now()
        twist_msg.linear.x = 0.0      # m/s
        twist_msg.angular.z = 0.0     # rad/s
        while rospy.Time.now() - start < rospy.Duration(1.0):
            # publish command
            self.pub_cmdvel.publish(twist_msg)
            rospy.sleep(0.05)


        self.say_voice("Carefull, Im moving backwards.", 0.0)
        if self.verbose: rospy.loginfo( "[Navigation_Test] Moving back." )

        start = rospy.Time.now()
        twist_msg.linear.x = -0.2       # m/s
        twist_msg.angular.z = 0.0     # rad/s
        while rospy.Time.now() - start < rospy.Duration(1.5):
            # publish command
            self.pub_cmdvel.publish(twist_msg)
            rospy.sleep(0.05)
        ...
        # Clear collision
        rospy.sleep(5)
        if self.verbose: rospy.loginfo( "[Navigation_Test] Clearing Collision" )
        self.under_collision = False
