...
CNavAssistant::CNavAssistant(std::string name) : Node("Nav_assistant_Server"), m_currentGoal(get_logger())
{
	...
	main_actionServer = rclcpp_action::create_server<NAS_ac::NavAssistant>(this, name,
...
void CNavAssistant::Init()
{
	...
	addpointService = create_service<NAS::NavAssistantPoint>("/navigation_assistant/add_point_of_interest", std::bind(&CNavAssistant::addPointCB, this, _1, _2));
...
void CNavAssistant::makePlan_async(const std::shared_ptr<rmw_request_id_t> header,
	const NAS::MakePlan::Request::SharedPtr request)
{
	...
	auto callback = [header, this](const rclcpp_action::ClientGoalHandle<GetPlan>::WrappedResult& w_result)
		{
