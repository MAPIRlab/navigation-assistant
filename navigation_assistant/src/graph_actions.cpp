 ...
 * CNP     -> Critical Navigation Point. Defines an area difficult for robot navigation and set two Intermediate Navigation Goals (ING) (i.e narrow
 * doors) CP      -> Critial Passage. Is a combination of passage (topo-localization) and CNP, also generating the 2 ING docking -> For Docking
 * Approach
 */
        ...
        for (auto it2 = it + 1; it2 != nodes_to_connect.end(); ++it2)
        {
            ...
            else
            {
                if (node_data[2] == "ING" && node_data2[2] == "ING")
                {
                    ...
                    if (verbose)
                        RCLCPP_WARN(get_logger(), "[NavAssistant]: NavDistance between Node[%s] <-> [%s] is %s (succes=%d)", node_data[0].c_str(),
                                    node_data2[0].c_str(), response->result[0].c_str(), response->success);

                    if (response->success)
                    {
                        // system("read -p 'Press Enter to continue...' var");
                        if (verbose)
                            RCLCPP_WARN(get_logger(), "[NavAssistant]: Adding new Arc between ING Nodes [%s] <-> [%s]", node_data[0].c_str(),
                                        node_data2[0].c_str());
                        // Create Arc
                        auto addArcRequest = std::make_shared<topology_graph::srv::Graph::Request>();
                        addArcRequest->cmd = "AddArc"; // params: [idfrom, idto, label, type, bidirectional]
